            ...
            return {
                "$or": [
                    {"organization_id": self.organization_id},
                    {
                        ...
                        "organization_slug": self.organization_slug or DEFAULT_ORGANIZATION_SLUG,
                    },
                    {
...
def ensure_device_in_tenant(device: Optional[dict], tenant: TenantContext) -> None:
    ...
    device_slug = device.get("organization_slug") or DEFAULT_ORGANIZATION_SLUG

    if tenant.organization_id and device_org_id:
    ...
    if tenant.organization_slug and device_slug != tenant.organization_slug:
        raise HTTPException(status_code=403, detail="Dispositivo fuera de tu organización")
