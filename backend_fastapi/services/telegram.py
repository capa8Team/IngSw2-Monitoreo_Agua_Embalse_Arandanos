...
class SensorAlertPayload:
    ...
    def __init__(self, deviceName: str, ph: float, temperature: float, conductivity: float, date: str, time: str):
...
class TelegramService:
    ...
    async def initialize(cls) -> bool:
        ...
        try:
            ...
            cls.application.add_handler(MessageHandler(filters.TEXT & ~filters.COMMAND, cls._handle_message))
    ...
    async def start_polling(cls) -> None:
        ...
        try:
            ...
            await cls.application.updater.start_polling(allowed_updates=Update.ALL_TYPES, drop_pending_updates=True)
    ...
    async def _handle_start(update: Update, context: ContextTypes.DEFAULT_TYPE):
        ...
        keyboard = InlineKeyboardMarkup([[InlineKeyboardButton("[ABRIR] Dashboard", url=settings.WEBAPP_URL)]])
    ...
    async def _handle_status(update: Update, context: ContextTypes.DEFAULT_TYPE):
        ...
        message = (f"[ESTADO] Tu Suscripcion\n\n{status_text}\nSuscriptores activos: {len(TelegramService.subscribed_chats)}\nTu ID: {chat_id}")
        await context.bot.send_message(chat_id=chat_id, text=message)
    ...
    async def _handle_message(update: Update, context: ContextTypes.DEFAULT_TYPE):
        await context.bot.send_message(chat_id=update.effective_chat.id, text="[INFO] Usa /help para ver los comandos disponibles.")

    @classmethod
    async def send_alert(cls, payload: SensorAlertPayload) -> dict[str, Any]:
        if not cls.bot: return {"status": "error", "message": "Bot no inicializado"}
        if not cls.subscribed_chats: return {"status": "ignored", "reason": "no-subscribed-chats"}
        if not cls._is_alert_condition(payload): return {"status": "ignored", "reason": "values-in-range"}

        message = cls._build_alert_message(payload)
        keyboard = InlineKeyboardMarkup([[InlineKeyboardButton("[VER] Dashboard", url=settings.WEBAPP_URL)]])
        sent, failed = 0, 0
        ...
        return {"status": "ok", "sent": sent, "failed": failed, "subscribers": len(cls.subscribed_chats), "timestamp": datetime.now().isoformat()}

    @classmethod
    async def send_message(cls, chat_id: int, message: str) -> bool:
        if not cls.bot: return False
        try:
    ...
    def _is_alert_condition(payload: SensorAlertPayload) -> bool:
        return (payload.ph < 6.0 or payload.ph > 8.5) or (payload.temperature < 5.0 or payload.temperature > 35.0) or (payload.conductivity < 100.0 or payload.conductivity > 2000.0)
    ...
    def get_stats(cls) -> dict[str, Any]:
        return {"subscribed_chats": len(cls.subscribed_chats), "subscribers": sorted(list(cls.subscribed_chats)), "bot_initialized": cls.bot is not None}

# Funciones de conveniencia
async def initialize_telegram() -> bool: return await TelegramService.initialize()
async def start_telegram_polling() -> None: await TelegramService.start_polling()
async def send_sensor_alert(payload: SensorAlertPayload) -> dict[str, Any]: return await TelegramService.send_alert(payload)
async def send_telegram_message(chat_id: int, message: str) -> bool: return await TelegramService.send_message(chat_id, message)
def get_telegram_subscribers() -> set[int]: return TelegramService.subscribed_chats.copy()
def get_telegram_stats() -> dict[str, Any]: return TelegramService.get_stats()